...
The alarm itself is evaluated each X seconds, where X is equal to **Period** or the granularity of the metric or expression, whichever is smaller\. For example, for EC2 instance, if deailed monitoring is disabled and **Period** is greater or equal to 5 minutes, the alarm is evaluated each 5 minutes, as the granularity of the metrics that EC2 provides\. If however, **Period** is less than 5 minutes, the alarm is evaluated each **Period** seconds\. Note that in this case, one should consider setting **Evaluation Periods** to be equal or greater to 5 minutes divided by **Period**, to avoid prolonged periods of `INSUFFICIENT_DATA` states\.
