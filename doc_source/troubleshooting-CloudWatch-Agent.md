...
|  The JSON file used to configure the agent, immediately after the wizard creates it\. For more information, see [Create the CloudWatch Agent Configuration File](create-cloudwatch-agent-configuration-file.md)\. |  /opt/aws/amazon\-cloudwatch\-agent/etc/config\.json   |  $Env:ProgramData\\Amazon\\AmazonCloudWatchAgent\\config\.json  | 
